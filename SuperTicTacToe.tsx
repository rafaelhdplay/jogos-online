...
const SuperTicTacToe: React.FC = () => {
  ...
  const renderCell = (boardIdx: number, cellIdx: number) => {
    ...
    if (value === 'X') {
      ...
      if (gameState.currentPlayer === 'X') {
        content = <SketchX className="w-full h-full p-1 text-white/40" strokeWidth={3} />;
      } else {
        content = <SketchO className="w-full h-full p-1 text-white/40" strokeWidth={3} />;
      }
    }
    ...
    return (
      <button
        ...
        className={`
          ...
          ${cellIdx < 6 ? 'border-b' : ''}
          ${isPlayable && isHumanTurn ? 'hover:bg-blue-600/40 cursor-pointer' : 'cursor-default'}
  ...
  const renderMiniBoard = (boardIdx: number) => {
    ...
    // Dynamic classes
    let containerClass = '';
    // We default to a darker gray border for inactive boards
    let borderClass = 'border-slate-700';

    if (isProjectedTarget) {
       // Highly visible target indicator (Grey/White flash)
       containerClass = 'bg-slate-700/80 ring-2 ring-slate-400 z-10'; 
       borderClass = 'border-slate-500';
    } else if (isNext) {
       // Highly visible active indicator (Neon Blue)
       containerClass = 'bg-blue-900/40 shadow-[inset_0_0_25px_rgba(59,130,246,0.3)] ring-2 ring-blue-500 z-0';
       borderClass = 'border-blue-500/50';
    } else {
       // Inactive
       containerClass = 'bg-[#121212] opacity-80';
    }
  ...
  return (
    <div className="flex flex-col items-center min-h-full w-full text-slate-200 selection:bg-red-500/30">
      ...
      <div className="w-full max-w-4xl px-4 py-3 text-xs md:text-sm text-slate-500 flex justify-start">
        <Link to="/" className="underline cursor-pointer hover:text-slate-300">Página Inicial</Link> 
